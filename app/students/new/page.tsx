  ...
  return (
    <div className="flex">
      ...
      <div className="flex-1 bg-cream">
        <PageHeader title="Add Student" subtitle="Enroll a new student and set their baseline record." />
          ...
          <Card className="p-6">
            <form action={createStudent} className="space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-[10px] font-semibold text-navy mb-1">Grade Level</label>
                  <select name="grade_level" required className="w-full border border-hairline rounded px-3 py-2 text-sm">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[10px] font-semibold text-navy mb-1">Enrollment Type</label>
                  <select name="enrollment_type" className="w-full border border-hairline rounded px-3 py-2 text-sm">
              ...
              <div>
                <label className="block text-[10px] font-semibold text-navy mb-2">EDGE Interests</label>
                <div className="grid grid-cols-2 gap-2">
                  {Object.entries(EDGE_PATHWAY_LABELS).map(([value, label]) => (
                    <label key={value} className="flex items-center gap-2 text-sm border border-hairline rounded px-3 py-2">
                      <input type="checkbox" name="edge_interests" value={value} className="rounded border-hairline" />
                      {label}
                    </label>
                  ))}
                </div>
...
function Field({
  ...
  return (
    <div>
      ...
      {textarea ? (
        <textarea name={name} rows={2} className="w-full border border-hairline rounded px-3 py-2 text-sm" />
      ) : (
        <input
          name={name}
          type={type}
...
function Checkbox({ label, name }: { label: string; name: string }) {
  return (
    <label className="flex items-center gap-2 text-sm text-navy">
      <input type="checkbox" name={name} className="rounded border-hairline" />
      {label}
    </label>
  );
}
